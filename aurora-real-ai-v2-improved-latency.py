...
class SingletonLLM:
    ...
    def generate_response(self, prompt, max_length=150, temperature=0.7, repetition_penalty=1.2, top_k=50, top_p=0.9):
...
def transcribe_audio(audio_filename):
    with (wave.open(audio_filename, "rb") as audio_file):
        if audio_file.getnchannels() != 1 or audio_file.getsampwidth() != 2 or audio_file.getframerate() not in [8000, 16000, 32000, 44100]:
            print("Audio file must be WAV format mono PCM.")
            sys.exit(1)
        audio_recognizer = KaldiRecognizer(model, audio_file.getframerate())
        final_transcription = ""
        while True:
            ...
            if audio_recognizer.AcceptWaveform(data):
                result = audio_recognizer.Result()
                final_transcription += eval(result)['text'] + " "
        final_transcription += eval(audio_recognizer.FinalResult())['text']
